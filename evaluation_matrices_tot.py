...
reference_file = "tot_generated_music_energetic.wav"  # Use ToT-generated energetic music as the reference file
generated_files = {
    "Happy Music": "GoT_generated_music_happy.wav",
    "Chatbot Music": "chat_bot_music.wav",
}

# Ground truth labels (e.g., "happy", "calm")
true_labels = ["happy", "calm"]
...
for label, file_path in generated_files.items():
    ...
    if mse is not None:
        ...
        if mse < 100:  # Example threshold for "happy"
...
conf_matrix = confusion_matrix(true_labels, predicted_labels, labels=["happy", "calm"])
class_report = classification_report(true_labels, predicted_labels, labels=["happy", "calm"], zero_division=0)
...
output_dir = "evaluation_results_tot"
os.makedirs(output_dir, exist_ok=True)

with open(f"{output_dir}/evaluation_results_tot.txt", "w") as f:
...
sns.heatmap(conf_matrix, annot=True, fmt='d', cmap='Blues',
            xticklabels=["Happy", "Calm"],
            yticklabels=["Happy", "Calm"])
